...
bool InputManager::Attach(int input) {
	for (unsigned int i = 0; i < m_RegisterInput.size(); i++) {
		if (input == m_RegisterInput[i]) { return false; }
	}
...
void InputManager::SetListeners(InputListener * listener) { m_Listeners.push_back(listener); }