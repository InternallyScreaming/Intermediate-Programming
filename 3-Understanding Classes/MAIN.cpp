...
*Tiffany Wiebe
*
*Objective: Create a header file to store necessary libraries and a class with member variables.
*The program must also have an array of type class to store 3 objects of that class.
*Three variables of the class must have a getter and setter that allows the user to get and set those variables for each of the 3 objects.
*/

//include header files so code doesn't break
#include "TiffanyWiebe_Assignment_03_UNIT.h"
#include <time.h>

//menu-like funtion prototypes
...
//Puts the cursor at the coordinates (x, y)
//Function provided by our wonderful professor
...
int main() {
	...
	//constant int varaibles to set the size of the arrays
	//allows for easy changes of numbers without the program freaking out
	const int amountOfMonsters = 3;
	const int amountOfPartyMembers = 3;

	//monster and party member (pm) arrays of the size of the const ints
	...
	//allows the user to see the introduction screen and the capability of moving on when they desire
	system("pause");
	//clear console screen
	system("CLS");

	//call monster function to input all the monster values
	monsters(Monsters, amountOfMonsters);
	//call (pm) function to input all (pm) values
	partyMembers(Party, amountOfPartyMembers);

	//infinite loop set to a boolean variable set to true
	while (running) {
		//calling the option menu
		optionMenu();
		//promting the user to pick an option
		cin >> option;

		switch (option) {
		//Option 1 is the printing values function
		case 1:
			//clear the consol
			system("CLS");
			//shows further options if user picks 1
			...
			//pause the system so the user can see what was printed
			system("pause");
			break;
